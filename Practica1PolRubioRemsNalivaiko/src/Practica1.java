// test
import Keyboard.*;
import java.util.Random;

public class Practica1 {

	public static void main(String[] args) {
		int j, k, n, v = 0;
		boolean f = false, e = true;
		Random random = new Random();

		int d = intSense("Indica de quines dimesions vols el taulell", 5, 15);

		boolean[][] joc = totFalse(d);
		boolean[][] aux = totFalse(d);

		int m = intSense("Indica quants organismes vius hi vols posar", 1, d * d);

		for (int i = 0; i < m; i++) {
			do {
				j = random.nextInt(d);
				k = random.nextInt(d);
			} while (joc[j][k]);
			joc[j][k] = true;
			System.out.println("Organisme a la posici�: " + j + "," + k);
		}
		System.out.println("Inicialment");
		System.out.println("***********");
		System.out.println(mapa(joc));

		int g = intSense("Indica quantes generacions vols", 1, 10);
		int[] vius = new int[g];

		for (int i = 0; i < g; i++) {
			System.out.println("Evoluci�: " + (i + 1));
			System.out.println("***********");

			for (int x = 0; x < d; x++) {
				for (int y = 0; y < d; y++) {
					n = numVeins(joc, d, x, y);
					System.out.print(n + " ");

					if (!joc[x][y] && n == 3)
						aux[x][y] = true;
					else if (joc[x][y] && (n < 2 || n > 3))
						aux[x][y] = false;
					else
						aux[x][y] = joc[x][y];
					if (aux[x][y])
						v++;
				}
				System.out.println();
			}
			vius[i] = v;
			if(v==0) {e = false; i = g;}
			v = 0;
			System.out.println();
			joc = aux;
			System.out.println(mapa(joc));
		}

		if (e) {
			v = 0;
			n = 0;
			for (int i = 0; i < g; i++) {
				System.out.print("Evoluci�: " + (i+1) + "--> hi ha " + vius[i]);
				if (vius[i] == 1)
					System.out.println(" organisme");
				else
					System.out.println(" organismes");
	
				if (vius[i] > v) {
					v = vius[i];
					n = i;
					f = false;
				} else if (vius[i] == v)
					f = true;
			}
			System.out.println();
			if (f)
				System.out.println("Hi ha m�s d'una evoluci� amb la mateixa vida ("
									+ v + "organismes. Una d'elles �s la" + (n+1));
			else
				System.out.println("L'evoluci� " + (n+1) + " �s la que m�s vida a tingut,"
									+ " amb un total de " + v + " organismes");
		}
		else System.out.println("La vida s'ha acabat");

	}

	private static int intSense(String pregunta, int m, int M) {
		int r;
		do {
			System.out.println(pregunta + " [" + m + "," + M + "]");
			r = Keyboard.readInt();
		} while (r < m || r > M);
		return r;
	}

	private static boolean[][] totFalse(int d) {
		boolean[][] a = new boolean[d][d];

		for (int i = 0; i < d; i++) {
			for (int j = 0; j < d; j++)
				a[i][j] = false;
		}
		return a;
	}

	private static String mapa(boolean[][] mapa) {
		String m = "";
		for (int i = 0; i < mapa.length; i++) {
			for (int j = 0; j < mapa[0].length; j++) {
				/*
				 * Aquests car�cters potser han de ser modificats per tal de tenir una correcta
				 * impressi� per consola si es canvia d'ordinador: - viu: 0x25A0 - mort: 0x25A1
				 */
				if (mapa[i][j])
					m += (char) 0x25A0;
				else
					m += (char) 0x25A1;
				m += " ";
			}
			m += "\n";
		}
		return m;
	}

	private static int numVeins(boolean[][] joc, int d, int x, int y) {
		int n = 0;

		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				if (x + i < 0 || x + i > d - 1 || y + j < 0 || y + j > d - 1)
					;
				else if (joc[x + i][y + j])
					n++;
			}
		}

		if (joc[x][y])
			n -= 1;

		return n;
	}
}
